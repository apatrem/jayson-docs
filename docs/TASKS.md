...
- **Outputs:**
  ...
  - `tests/fixtures/m7-single-section-proposal.yaml` (NEW) — a single-section subset of `examples/sample-proposal.yaml` (extract just section 1: "Executive summary"). Includes one of each block type the renderer handles end-to-end: 1 prose, 1 callout, 1 chart, 1 KPI-cards. NO image yet (T-123e adds the image when it lands the inlining IPC; T-123d threads it through). This fixture is the canonical M7-spike-acceptable single-section doc; T-123d will swap the integration harness onto it.
...
- **Outputs:**
  - `tests/fixtures/m7-single-section-proposal.yaml` — **extend** the fixture already created in T-123b by adding one image block (`type: image, src: "assets/team-meeting.jpg"`) and one diagram block (Mermaid). This is the addition that exercises T-123e's image-inlining path + the diagram SSR path that wasn't yet meaningful in T-123b's constraint test. The fixture now contains: 1 prose, 1 callout, 1 chart, 1 KPI-cards (from T-123b) + 1 image + 1 diagram (added here). Document the extension at the top of the fixture file: `# Extended in T-123d to add image + diagram (T-123e image inlining + real renderer test)`.
