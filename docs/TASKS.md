...
- **Depends-on:** T-123
- **Reads:** `src/ui/views/DocumentView.tsx` (esp. `documentToEditorContent` + `editorContentToDocument`), `examples/sample-proposal.yaml` (the canonical multi-section fixture this constraint protects against), `BLOCKERS.md` §[drift-2026-05-26d] (existing drift entry — already appended by the 2026-05-26 planning commit `4a670c5`)
- **Outputs:**
  ...
  - `tests/ui/views/DocumentView.test.tsx` — assert that opening a multi-section doc renders the constraint message + Back-to-welcome button. Also assert that the new `tests/fixtures/m7-single-section-proposal.yaml` fixture (loaded via `readFileSync`) opens and renders normally with no constraint message.
- **Acceptance:** opening `examples/sample-proposal.yaml` (4 sections) in M7-spike shows the constraint message, not corrupted save behavior. Opening `tests/fixtures/m7-single-section-proposal.yaml` (1 section) renders normally. The existing BLOCKERS.md drift entry `[drift-2026-05-26d]` is verified accurate; do NOT re-append (it was already added by commit `4a670c5`).
- **est.** 1h
- **Note: confirmed by code review as BLOCKER-2 + verified the M7 harness uses a synthetic single-section doc that masks the bug. M8 T-126's router refactor + section-aware mapping is the proper fix. The fixture file is owned by T-123b (this task) rather than T-123d because the constraint test needs a single-section reference doc to pass — moving it from T-123d to T-123b breaks the original circular dependency (T-123d → T-123b → fixture from T-123d).**
