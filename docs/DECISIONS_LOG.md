...
## D26 — Comfortable-fill bands: per-(layout, region) fill targets derived from master geometry, with a symmetric under-fill warning

**Decided:** add a third density signal beneath D23's two-tier caps — a **comfortable-fill band**, a two-sided target (`lower..upper`) computed **per (layout, region)** from the box's real geometry (`src/setup/layout-spec.json` `x/y/w/h`, D19) and its effective font size. Setup derives it with an **analytic line model**: `lines = ⌊h / (pt · 1.2)⌋`, `charsPerLine = ⌊w / (pt · 0.5)⌋`, capacity in a unified line-currency, **band = `[~0.55, ~0.85] × capacity`** (words for prose regions, items for bullet regions). **Font size is sourced hybrid:** the run's explicit `sz` when present, else the placeholder → layout → master style cascade, else a per-region-kind default seeded from the master's discrete **8 / 12 / 18 / 22 pt** palette. The band is **LLM-facing** in `skills/report-pptx/layout-catalogue.json`, **dual-homed** with `src/schema/caps.ts`, and kept honest by the same **drift test** as D23's caps. Unlike D23's `optimal`/`max` (keyed by region *kind*, and therefore footprint-blind), the band is keyed by the **box** — so a full-width body and a half-width column get different targets. The CLI emits a **symmetric under-fill soft-warning** (exit 0, stderr) when content falls below the band's lower bound — the mirror of D23's over-optimal warning; the per-kind hard `max` remains the only Zod-rejecting ceiling. The `report-pptx` skill instructs the LLM to **match content volume to a layout's fill-band** when selecting a layout — which is how "how much text / what size?" is answered **without** ever letting the LLM choose a font (the master stays canonical — D2-2; SKILL.md §1 hard rule intact).
...
**Cardinality policy:** author **3 / 4 / 5** per linear archetype (process, KPI, funnel, feature-grid, roadmap); **3 / 4** for pyramid; **fixed-4** for the matrix (+ optional 9-box); **singular** for big-number and quote; **growing** for the three tables — **≈36 new layouts**. **Authoring & brand fidelity:** new layout slides are **drafted to the master's existing theme** (its accent palette, the 8/12/18/22 type scale, its rectangle/accent styles — never invented styling), then pass a **brand sign-off gate** before the `layoutId` enters the catalogue (lightweight for the public *Acme* placeholder master — a visual diff against the existing 26; a firm designer's nod on a real install). Each archetype then follows the standard add-a-layout path (SLIDE_LAYOUT_LIBRARY "Adding a new layout": design → name shapes → regenerate `layout-spec.json` → Zod schema + catalogue entry + fixture).
