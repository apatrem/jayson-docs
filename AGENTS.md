...
6. **Effort/review dial — `mode: low | medium | hard`, default `low` (prefer low, justify higher).** The routine ~90% path is `low`: one implementer — **Cursor Composer** by default (chosen per session in Superset; the orchestrator role stays `claude-code` — `docs/adr/0002-buy-engine-build-conventions.md`) — + the gate + one adversarial reviewer. `medium` adds an independent dual review on the PR (GPT-5.5 xhigh + Opus 4.8 xhigh → synthesis; `/agentic-workflow:review`). `hard` runs competitive best-of-N across lineages → an Opus smart-merge → then the medium dual review (`hard ⊇ medium`). Agent biases for best-of-N: `ROLES.md`. Effort dial: `docs/adr/0004-effort-solo-default.md`.
