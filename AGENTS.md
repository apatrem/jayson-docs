...
6. **Effort/review dial — `mode: low | medium | hard`, default `low` (prefer low, justify higher).** The routine ~90% path is `low`: one implementer — **Cursor Composer** by default (chosen per session in Superset; the orchestrator role stays `claude-code` — `docs/adr/0002-buy-engine-build-conventions.md`) — + the gate + one adversarial reviewer. `medium` adds an independent cross-lineage dual review on the PR → synthesis (`/agentic-workflow:review`). `hard` runs competitive best-of-N over two lineages → smart-merge → then the medium dual review with **≥1 structurally-clean lens** (`hard ⊇ medium`). **The model→role→tier picks live in `docs/MODELS.md` (single source of truth)** — the current policy is **GPT-5.5 primary**, **Opus 4.8 the Claude medium lens**, **Fable 5 only the optional `hard` 3rd lens** (Fable-first is retired — ADR-0004 Update). Agent biases for best-of-N: `ROLES.md`. Effort dial: `docs/adr/0004-effort-solo-default.md`.
7. **`mode` is a floor; risk raises it (ADR-0004 refinement 3).** A change touching protected/destructive surface — `rm -rf`/in-place rewrites, the gate or CI config, lockfiles/deps, migrations·schema·the Zod layout schemas/`layout-spec.json`, auth/secrets, public API or the fill-plan contract — is forced to **≥ `medium`** regardless of the declared mode. The orchestrator bumps the tier and records *"escalated by risk floor"* in the task and PR. (Reactive escalation on findings is ADR-0010.)
8. **Process model:** one page in `docs/WORKFLOW.md`; workflow ADRs in `docs/adr/`.
...
- **Smallest correct change — as a ladder (ADR-0011).** Before writing code, walk: *is it needed at all? → stdlib → platform → an already-installed dep → one line → minimal code.* Paired with a **minimalism floor that is never cut**: input validation at trust boundaries (here: **fill-plan schema validation** and the **reject-don't-fix** error policy, `ERROR_HANDLING.md`), error handling that prevents data loss, security, and accessibility. Minimalism never trades these away.
- **Mark deliberate shortcuts inline.** When you knowingly cut a corner, leave a `// SHORTCUT(<ceiling>): <upgrade path>` marker naming the known ceiling **and** how to lift it — e.g. `// SHORTCUT(O(n²) scan): ok <1k rows; add an index if it grows`. The code is the ledger; `grep -rn 'SHORTCUT('` is the running inventory (no `DEBT.md` file). The `medium`/`hard` reviewer enforces these and lists the SHORTCUTs each PR adds (ADR-0011).
...
- **Sparse review.** Ask a reviewer (Codex/Claude) for **blockers only** — correctness, security, missing tests, broken boundaries; ≤10 findings, ranked by severity. Lint/format handles style. `hard`-tier best-of-N biases: `ROLES.md`. The `medium`/`hard` reviewer also runs an **advisory minimalism lens** (over-engineering delete-list; ensures each deliberate corner carries a `SHORTCUT(…)` marker) — advisory, never a blocker class (ADR-0011).
- **Minimalism is legible, not tracked out-of-band.** Deliberate corners carry `// SHORTCUT(<ceiling>): <upgrade path>` markers; `grep -rn 'SHORTCUT('` is the ledger (ADR-0011, §3). The minimalism floor (validation / data-loss-safe error handling / security / accessibility) is never cut.
- **Risk floor on the dial.** Protected/destructive surface forces the task to **≥ `medium`** regardless of declared mode (§0 rule 7; ADR-0004 refinement 3).
