...
export const ChartDataPanel: React.FC<ChartDataPanelProps> = ({ block, onUpdate, onClose }) => {
  ...
  return (
    ...
    >
      ...
      {/* Takeaway */}
      <Field label="Takeaway (the 'so what')" error={errors["takeaway"]}>
        <textarea
          rows={2}
          value={draft.takeaway ?? ""}
          onChange={(e) => updateField("takeaway", e.target.value || undefined as never)}
          maxLength={200}
        />
      </Field>
...
const ChartDataGrid: React.FC<ChartDataGridProps> = ({ data, onChange }) => {
