/**
 * Reference block #5 — Chart
 *
 * The HARDEST block in the catalogue. Demonstrates patterns that the simpler
 * blocks (callout, prose) don't exercise:
 *   - Complex nested data structures (series × values).
 *   - Variant logic (chart type → which fields are required).
 *   - Side-panel editing UI (D-24) — not inline.
 *   - SSR pre-rendering for PDF export (ECharts via Node-side `setOption`).
 *
 * Production path: src/schema/blocks/chart.ts
 */
...
/**
 * The chart's data payload. All numeric data is stored here; no live data
 * connections (R8).
 *
 * Note on schema-shape choices:
 *  - `xLabels` is required for bar/line/area/waterfall (the x-axis category labels).
 *  - For pie/donut, treat xLabels as the slice labels; series[0].values as slice sizes.
 *  - For scatter, the schema is overloaded — store {x, y} pairs in series.values as
 *    a flat array [x1, y1, x2, y2, ...] (validator does the parity check).
 *  - For mekko, the y-axis is implied by the values; xLabels are the column labels.
 */
...
export const ChartAxesSchema = z.object({
  ...
  /** If true, y-axis starts at zero. Default depends on chart type (true for bar/area; false for line). */
...
export const ChartBlockSchema = BlockBaseSchema.extend({
  ...
  .strict()
  // Cross-field validation: pie/donut don't use axes; scatter needs paired values.
