/**
 * Reference block #5 — Chart tests.
 *
 * Same 5 test layers as callout (schema/renderer/mapping/editor) — plus an
 * extra layer for the side panel + Excel paste.
 *
 * Pattern notes for copy-adapt:
 *  - The chart test file is bigger than callout's because the schema is
 *    larger (cross-field validation, multiple chart types, scatter parity).
 *  - The renderer test uses an in-memory ECharts instance to verify the
 *    option object — we don't snapshot the rendered SVG (too brittle).
 */
...
import { getEChartsOption } from "./Chart";
import {
  ChartTipTapNode,
...
const testBrandMin = {
  ...
  colors: {
    chartPalette: {
      qualitative: ["#0B3D91", "#E8A33D", "#2D9C5A", "#C44536",
...
// ── 2. Schema — invalid fixtures + cross-field rules ────────────────────────
...
describe("ChartBlockSchema — cross-field validation", () => {
  ...
  it("rejects a scatter chart with odd-length values", () => {
    const bad: ChartBlock = {
      ...
      chartType: "scatter",
      data: {
        series: [{ name: "Points", values: [1, 2, 3] }],   // odd length
      },
    };
  ...
  it("rejects a bar chart missing xLabels", () => {
    const bad = { ...minimalBarChart, data: { series: minimalBarChart.data.series } };
    const result = ChartBlockSchema.safeParse(bad);
    expect(result.success).toBe(false);
  });
  ...
  it("rejects unknown chart types", () => {
    const bad = { ...minimalBarChart, chartType: "treemap" as never };
    expect(ChartBlockSchema.safeParse(bad).success).toBe(false);
  });
});

// ── 3. Renderer (option-builder) — uses brand tokens ────────────────────────
...
describe("Chart mapping (DocModel <-> ProseMirror)", () => {
  it("round-trips a minimal bar chart losslessly", () => {
    const pm = chartBlockToProseMirror(minimalBarChart);
    const back = proseMirrorToChartBlock(pm as never);
    expect(back).toEqual(minimalBarChart);
  });

  it("round-trips a fully-populated stacked-bar chart losslessly", () => {
    const pm = chartBlockToProseMirror(stackedBarChart);
    const back = proseMirrorToChartBlock(pm as never);
    expect(back).toEqual(stackedBarChart);
  });

  it("round-trips a pie chart losslessly", () => {
    const pm = chartBlockToProseMirror(validPieChart);
    const back = proseMirrorToChartBlock(pm as never);
    expect(back).toEqual(validPieChart);
  });
});
...
describe("Chart TipTap node", () => {
  it("registers an `insertChart` command", () => {
    const editor = new Editor({ extensions: [StarterKit, ChartTipTapNode] });
